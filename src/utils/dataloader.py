...
class KLUEDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    data = {key: val[item_idx].clone().detach() for key, val in self.dataset.items()}
    if self.labels:
...
class Dataloader(pl.LightningDataModule):
  ...
  def __init__(
    ...
    self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, max_length=self.max_length)
  ...
  def train_dataloader(self):
    return self.dataloader if self.dataloader \
  ...
  def val_dataloader(self):
    return self.dataloader.val_dataloader() if self.dataloader \
                                            else DataLoader(self.val_dataset, 
                                                            batch_size=self.batch_size, 
                                                            num_workers = 4, 
                                                            collate_fn = default_collate)
