...
def execute_kfold_train(conf, version, is_monitor, is_scheduler):
    from train.train_kfold import main
    
    main(conf, version, is_monitor, is_scheduler)
    return

def execute_stratified_train(conf, version, is_monitor, is_scheduler):
...
def main():
    ...
    if args.option == "train":
        ...
        execute_kfold_train(conf, args.version, args.is_monitor, args.is_scheduler)
    elif args.option == 'train_stratified':
        execute_stratified_train(conf, args.version, args.is_monitor, args.is_scheduler)
