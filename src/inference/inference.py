...
def main(conf, version, model_path, is_checkpoint=False):
    ...
    pred_answers = num_to_label(predictions, conf.num_to_label_dict_path) # 숫자로 된 class를 원래 문자열 라벨로 변환.
    print_msg('예측이 완료되었습니다', 'INFO')
    
    # submission 파일 저장
    print_msg(f'submission 파일을 저장합니다....', 'INFO')
    output = pd.read_csv(conf.submission_path)
    output = pd.DataFrame({'id':output['id'],'pred_label':pred_answers,'probs':probabilities.tolist(),})
    file_name = make_file_name(conf.model_name.replace('/', '_'), version=version, format='csv')
    save_path = os.path.join(save_path, file_name)
    output.to_csv(save_path, index=False)
    print_msg(f'submission 파일 저장을 완료하였습니다....', 'INFO')
